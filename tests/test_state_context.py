...
class StateContextTests(unittest.TestCase):
    def test_history_is_loaded_before_new_review_and_full_review_is_due(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(len(portfolio_nav_history(snapshot_path, resolution_path=Path(directory) / "missing-resolutions.jsonl")), 2)
            self.assertIsNotNone(last_full_review(decision_path))
            context = build_history_context(snapshot_path, decision_path, metrics_path=Path(directory) / "metrics.jsonl", as_of="2026-01-16T00:00:00Z", cash_flow_resolution_path=Path(directory) / "missing-resolutions.jsonl")
